...
public class BoardSearchImpl extends QuerydslRepositorySupport implements BoardSearch {
    ...
    public Page<Board> search1(Pageable pageable) {
        ...
        // JPQLQuery 객체 생성 - from() 메소드를 통해 쿼리 시작
        // from() 메소드에는 Q도메인 클래스를 전달
        // from(board) : board 테이블을 대상으로 쿼리 생성
        JPQLQuery<Board> query = from(board);

        // BooleanBuilder 객체 생성 - where 절에 들어갈 조건 생성
        BooleanBuilder booleanBuilder = new BooleanBuilder();

        // where 절에 들어갈 조건 설정, or 조건으로 설정
        booleanBuilder.or(board.title.contains("tit"));
         ...
         * 페이징
         * query : JPQL 쿼리객체
         * this.getQuerydsl() : 이 메소드는 Querydsl 객체를 반환. Querydsl 객체는
         *      QueryDSL 쿼리를 작성하고 실행하는 데 필요한 유틸리티 메소드를 제공.
         * applyPagination() : Querydsl에 Pageable 적용하여 페이징 처리하는 역할.
         * 이 메소드는 Querydsl 클래스에서 제공하는 메소드로 Pageable 객체를 사용하여
         * 쿼리에 페이징 조건을 적용. 여기서 Pageable은 Spring Data JPA에서 페이징
         * 정보를 담고 있는 인터페이스.
         */
    ...
    /**
     * 게시물 목록과 그 게시물이 갖고 있는 댓글 List 함께 조회
     * -
     * @param types
     * @param keyword
     * @param pageable
    ...
    public Page<BoardListAllDTO> searchWithAll(String[] types,
        ...
        // querydsl 객체지향 쿼리 객체 생성을 위한 Qtype domain 지정
        QBoard qBoard = QBoard.board;
        QReply qReply = QReply.reply;

        // JPQLQuery 객체 초기화(Board 엔티티를 대상으로 쿼리를 시작합니다.)
        JPQLQuery<Board> queryObj = from(qBoard);

        //left join(댓글의 게시글 번호와 게시글의 번호가 같아야)
        // Reply 엔티티와 Board 엔티티를 left join하여 Reply가 Board와 연결 조건 설정.
        queryObj.leftJoin(qReply).on(qReply.board.eq(qBoard));

        // 검색 조건을 동적으로 처리하기 위해 BooleanBuilder 사용
        if( (types != null && types.length > 0) && keyword != null ){
            BooleanBuilder booleanBuilder = new BooleanBuilder(); // (
            for(String type: types){
                switch (type){
        ...
        // 게시물당 이미지갯수 count()해야 해서 게시물로 그룹바이 설정
         ...
         * ]
         */
        // Tuple은 QueryDSL에서 여러 타입의 데이터를 함께 반환할 때 사용하는 클래스입니다.
        //여기서는 Tuple을 사용하여 Board 엔티티와 각 Board에 대한 댓글 수를 함께 반환하도록 쿼리를 작성합니다.
        // queryObj는 이미 생성된 JPQLQuery<Board> 객체로 이 객체는 기본적인 쿼리 설정이 완료된 상태.
        // select() 메소드를 통해 쿼리의 결과를 설정할 수 있음. select 메소드를 사용하여 쿼리의 반환 형식을 지정합니다.
        // 여기서는 qBoard와 qReply.countDistinct()를 선택합니다.
        // qBoard는 Board 엔티티를 나타내며, qReply.countDistinct()는 각 Board에 대한 댓글의 개수를 나타냄
        ...
        List<BoardListAllDTO> dtoList = tupleList.stream().map(tuple -> {
            ...
            // 게시물 엔티티
            Board board = tuple.get(0, Board.class);    // 첫번째 tuple에서 0번째값(Board), 1번째값(Long)
