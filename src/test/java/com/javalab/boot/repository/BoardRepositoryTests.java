...
public class BoardRepositoryTests {
    ...
    public void testGetTime() {
        // When: getTime 메소드 호출
        String currentTime = boardRepository.getTime();

        // Then: 결과 검증
        log.info("현재 시간: " + currentTime);
        Assertions.assertNotNull(currentTime, "현재 시간이 null입니다.");

        // 추가적인 검증 로직 작성 (필요에 따라)
        // 예를 들어, 시간 형식을 검증하거나 특정 시간 범위 내에 있는지 확인할 수 있습니다.
    }
   ...
   //@Disabled
    ...
    public void testUpdate() {
        ...
        Long bno = 100L; // 실제 디비에 있는 번호 100번을 수정한다는 것을 세팅하는 것

        Optional<Board> result = boardRepository.findById(bno);

        Board board = result.orElseThrow(); // 값이 있으면 반환,없으면 예외 발생
    ...
    public void testPaging() {
        ...
        // findAll() 메소드에 페이징 객체를 전달하면 그 결과는 Page<T> 타입이다.
        // Page<T> 타입에는 페이징을 위한 모든 정보가 들어있다.
        ...
        List<Board> todoList = result.getContent(); // Board 타입의 리스트 얻기
